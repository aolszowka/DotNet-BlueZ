...
namespace ProrepubliQ.DotNetBlueZ.Extensions
{
    public static class Extensions
    {
        ...
        public static async Task<Device> GetDeviceAsync(this IAdapter1 adapter, string deviceAddress)
        {
            ...
            if (dev != null) return await Device.CreateAsync(dev);
            return null;
        }
        ...
        public static Task<IDisposable> WatchDevicesAddedAsync(this IAdapter1 adapter, Action<Device> handler)
        {
            async void OnDeviceAdded(
                (ObjectPath objectPath, IDictionary<string, IDictionary<string, object>> interfaces) args)
            {
        ...
        public static async Task<GattCharacteristic> GetCharacteristicAsync(this IGattService1 service,
        ...
        public static async Task<byte[]> ReadValueAsync(this IGattCharacteristic1 characteristic, TimeSpan timeout)
        {
            ...
            if (!readTask.IsCompleted) throw new TimeoutException("Timed out waiting to read characteristic value.");
        ...
        public static async Task WaitForPropertyValueAsync<T>(this IDevice1 obj, string propertyName, T value,
            TimeSpan timeout)
        {
            var (watchTask, watcher) = WaitForPropertyValueInternal(obj, propertyName, value);
            ...
            await Task.WhenAny(watchTask, Task.Delay(timeout));
