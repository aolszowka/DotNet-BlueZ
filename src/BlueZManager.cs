...
namespace ProrepubliQ.DotNetBlueZ
{
    public static class BlueZManager
    {
        ...
        {
            var adapters = await GetProxiesAsync<IAdapter1>(BluezConstants.AdapterInterface, null);
        ...
        {
            if (rootObject != null && !objectPath.ToString().StartsWith($"{rootObject.ObjectPath}/")) return false;
