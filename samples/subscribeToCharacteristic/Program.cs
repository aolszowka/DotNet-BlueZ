  ...
  subscribeToCharacteristic 8C:8E:F2:AB:73:76 hci0 CAFE CFFE (see https://github.com/vestervang/early-iOS-BluetoothLowEnergy-tests/tree/master/myFirstPeripheral)");
        ...
        private static async Task device_ServicesResolvedAsync(Device device, BlueZEventArgs e)
        {
            try
            {
                ...
                {
                    Console.WriteLine(
                        $"Characteristic UUID {s_characteristicUUID} not found within service {s_serviceUUID}.");
                    return;
                }
        ...
        {
            if (value.Length < 8) throw new ArgumentException("8 bytes are required for ANCS notifications.");

            var eventIds = new[] { "added", "modified", "removed" };
            var categoryIds = new[]
            {
                "Other", "IncomingCall", "MissedCall", "Voicemail", "Social", "Schedule", "Email", "News",
            ...
            Console.WriteLine(
                $"{categoryIds[value[2]]} notification {eventIds[value[0]]} (Count: {value[3]}) (UID: {BitConverter.ToString(notificationUid)})");
        }

        private static DateTime ReadCurrentTime(byte[] value)
        {
            if (value.Length < 7) throw new ArgumentException("7+ bytes are required for the current date time.");
